...
void	load_images(t_game *game)
{
	...
	game->assets[F1].img = mlx_xpm_to_image(game->mlx_ptr, BG_SPRITE,
			&game->assets[F1].width, &game->assets[F1].height);
	game->assets[W1].img = mlx_xpm_to_image(game->mlx_ptr, WALL_SPRITE,
			&game->assets[W1].width, &game->assets[W1].height);
	game->assets[P1].img = mlx_xpm_to_image(game->mlx_ptr, PLAYER_SPRITE,
			&game->assets[P1].width, &game->assets[P1].height);
	game->assets[P2].img = mlx_xpm_to_image(game->mlx_ptr, PE_SPRITE,
			&game->assets[P2].width, &game->assets[P2].height);
	game->assets[C1].img = mlx_xpm_to_image(game->mlx_ptr, COLLECT_SPRITE,
			&game->assets[C1].width, &game->assets[C1].height);
	game->assets[E1].img = mlx_xpm_to_image(game->mlx_ptr, EXIT_SPRITE,
			&game->assets[E1].width, &game->assets[E1].height);
	game->assets[E2].img = mlx_xpm_to_image(game->mlx_ptr, OE_SPRITE,
